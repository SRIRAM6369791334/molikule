    ...
    <section class="hw-section hw-ad">
        <div class="auto-container">
            <div class="hw-ad__grid">
                ...
                @php
                    $count = $bentoCards->count();
                    $bentoClasses = [];
                    for ($i = 0; $i < $count; $i++) {
                        if ($i === $count - 1) {
                            $bentoClasses[$i] = 'hw-bento-card--combo';
                        } else {
                            $bentoClasses[$i] = ($i % 2 === 0) ? 'hw-bento-card--freshener' : 'hw-bento-card--summer';
                        }
                    }
                ...
                @endphp
                @forelse($bentoCards as $index => $card)
                    @php
                        $class = $bentoClasses[$index] ?? 'hw-bento-card--freshener';
                        $delay = $index > 0 ? 'data-aos-delay="' . ($index * 100) . '"' : '';
                        $theme = $classThemes[$class];
                    @endphp
                    <div class="hw-bento-card {{ $class }}" data-aos="fade-up" {!! $delay !!}>
                        <div class="hw-bento-card__bg"
                            style="background-image: url('{{ $card->image_url }}');"></div>
                        <div class="hw-bento-card__overlay"></div>
                        <div class="hw-bento-card__content">
                            <span class="hw-bento-card__tag {{ $theme['tag'] }}">{{ $card->tag }}</span>
                            @php
                                // Protect the layout: remove any broken HTML tags the client might have typed
                                $safeTitle = strip_tags($card->title);
                                // Allow the client to easily make text green by typing *text*
                                $formattedTitle = preg_replace('/\*(.*?)\*/', '<em>$1</em>', $safeTitle);
                            @endphp
                            <h2 class="hw-bento-card__title {{ $theme['title'] }}">{!! $formattedTitle !!}</h2>
                            <p class="hw-bento-card__desc {{ $theme['desc'] }}">{{ $card->description }}</p>
                            
                            @if($index === 2)
                                <div class="hw-bento-card__buttons">
                                    <a href="{{ route('shop') }}" class="hw-btn {{ $theme['btn'] }} hw-btn--sm">Shop Now</a>
                                </div>
                            @else
                                <a href="{{ route('shop') }}" class="hw-btn {{ $theme['btn'] }} hw-btn--sm">
                                    Shop Now
                        ...
                        <div class="hw-bento-card__num {{ $theme['num'] }}">0{{ $index + 1 }}</div>
                    </div>
                @empty
            ...
            <div class="inner-container">
                <div class="row clearfix">
                    @forelse($brands as $brand)
                        <div class="col-lg-3 col-md-4 col-sm-12 brand-block">
